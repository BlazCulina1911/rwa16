...
<!-- First Grid -->
<div class="container" style="padding-top: 100px; padding-bottom: 71.4vh">
    <h1 style="text-align:center;">
{{--        Trenutno vrijeme:--}}

{{--        {{ \Carbon\Carbon::now('Europe/Zagreb')->format('h:i:s d-m-Y') }}--}}

    </h1>

    <h2>Odaberi datum</h2>
    <div style="position:relative; top: 30px; display: flex; flex-wrap: wrap; justify-content: center; gap: 20px; flex: 1;">
        @foreach(\App\Models\Reservation::displayDates() as $date)
            <a href="{{url()->current()}}/{{$date->toDateString()}}"><button value="">{{$date->format('d-m-Y')}}</button></a>
        @endforeach
    </div>
</div>
