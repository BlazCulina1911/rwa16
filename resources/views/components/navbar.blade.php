<div class="w3-top">
    <div class="w3-bar w3-red w3-card w3-left-align w3-large">
        ...
        <a href="{{ url('/') }}" class="w3-bar-item w3-button w3-padding-large {{ (request()->is('/')) ? 'w3-white' : ''}} ">Početna</a>
        <a href="{{ url('/contact') }}" class="w3-bar-item w3-button w3-hide-small w3-padding-large w3-hover-white {{ (request()->is('contact')) ? 'w3-white' : ''}} ">Kontakt</a>
        <a href="{{ url('/about') }}" class="w3-bar-item w3-button w3-hide-small w3-padding-large w3-hover-white {{ (request()->is('about')) ? 'w3-white' : ''}} ">O nama</a>
        @auth
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button class="w3-bar-item w3-button w3-padding-large w3-right" type="submit">Odjavi se</button>
            </form>
            <a href="{{ url('/reservations') }}" class="w3-bar-item w3-button w3-hide-small w3-padding-large w3-hover-white {{ (request()->is('reservations')) ? 'w3-white' : ''}}">Rezervacija</a>
            <a href="{{ url('/profile') }}" class="w3-bar-item w3-button w3-hide-small w3-padding-large w3-hover-white w3-right {{ (request()->is('profile')) ? 'w3-white' : ''}}">Moj Profil</a>
        @endauth
        @guest
